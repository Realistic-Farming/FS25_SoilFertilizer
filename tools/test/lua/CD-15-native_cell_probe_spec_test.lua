...
do
    ...
    g_terrainNode = 1
    local desc = { name = "WHEAT", terrainDataPlaneId = 42, startStateChannel = 0, numStateChannels = 4, cutStates = { [9] = true }, witheredState = 10 }
...
do
    local calls = {}
    local function native(self, workArea, dt) calls[#calls + 1] = { self, workArea, dt } return 7, 8 end
    ...
    logged = {}
    calls = {}
    local r1, r2 = workArea.processingFunction(vehicle, workArea, 33)
    T.eq("F6 on: the stored pointer delegates exactly once", #calls, 1)
    T.eq("F7 on: unchanged arguments", calls[1][3], 33)
    T.eq("F8 on: returns passed through", r1 .. "," .. r2, "7,8")
    ...
    T.eq("F12 off restores the instance slot", vehicle.processCutterArea, native)
