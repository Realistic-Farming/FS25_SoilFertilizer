...
### Fixed
- **RSF-F226: tedder drying and combine straw birth now actually run.** Tedding has never done anything in a shipped game: the hook logged a non-zero "patched" count at every load while the wrapper behind it never executed once. The same was true for straw laid down by a combine. Both wrappers now run for real. On default settings nothing changes yet, tedded grass drying and straw birth-tracking are part of the ground-material family, which stays behind Experimental Systems. Turn Experimental Systems on in the settings panel to see tedded grass dry faster than untedded grass, and combine-laid straw remember when it was cut. Nothing to configure otherwise, and no other machine (mowers, windrowers, balers, the combine's chopper mode) is affected.
