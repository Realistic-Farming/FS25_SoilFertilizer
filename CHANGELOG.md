...
### Fixed
- **RSF-F213: the ground membership index is rebuilt when it cannot be trusted, and empty ground leaves it.** Behind Experimental Systems, like the rest of the ground-material family. A saved index is now read only when it came from the same save as the soil condition it indexes; otherwise, or when its file is a different size, it is rebuilt from the condition and the material actually on the ground (a different size used to switch the daily weather back to the old per-field pass). The save rebuilds an index that missed a write before writing it. Ground that a failed machine pass marked but that holds nothing drops out of the index at load instead of being carried from save to save. (MAINTENANCE rows 107 and 108.)
