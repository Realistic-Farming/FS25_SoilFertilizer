...
### Fixed
- **Multiplayer security: the menu fungicide treatment acts only on your own or contracted fields, and only your farm pays.** A client could treat any field on the map from the scout dialog or the SoilTreat console command, and a sender the server could not place was billed to the host's farm. The treatment now takes the same standing as the field scout (your farm owns the land or is contracting its owner; unowned land is refused), on every door and on dedicated servers too, and the acting farm is charged with no fallback. Singleplayer: a field your farm does not own can no longer be treated from the menu. Other mods' own treatments that charge nobody (NPC Favor's neighbour treatments, ProStaff's disease flush) are unchanged.
