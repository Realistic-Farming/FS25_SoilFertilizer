...
### Fixed
- **Missing translations filled in, every language.** Fourteen texts the mod shows had no entry in some or all language files, so players saw English or the game's "Missing" sentence: the Set Field State and Set Field Disease page titles, the rotation panel's tips, heading and columns, the field-merge labels, the unknown-status word and the FIELD INFO rotation "N/A". Czech also gains the soil map legend and advice lines, the Experimental Systems dialog title and four report lines; French gains the sensor, See & Spray, variable-rate and report labels; Chinese (Simplified) gains the sensor labels. English text is unchanged. (MAINTENANCE row 89.)
